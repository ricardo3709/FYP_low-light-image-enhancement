...
def random_crop_from_folder(low_folder_path, crop_size):
    ...
    for prefix in ["0", "1"]:
        ...
        x_start_file = (start_x_lr // patch_size) * \
            patch_size  # x start index in file
        ...
        while x_start_file + i * patch_size < start_x_lr + crop_size:
            ...
            while y_start_file + j * patch_size < start_y_lr + crop_size:
                ...
                i_start_res, i_end_res = idx_res_func(
                    start_x_lr, x_start_file, i)
                j_start_res, j_end_res = idx_res_func(
                    start_y_lr, y_start_file, j)
...
def random_crop(img_list, size):
    ...
    for img in img_list:
        assert isinstance(
            img_list[0], torch.Tensor) and img.shape[2:] == shape[2:]
...
class RawImageDataset(Dataset):
    ...
    def __init__(self, root, transform=None, split="train", auto_padding_scale=None):
        ...
        for line in lines:
            line = line.rstrip()
            # "rgb_path, raw_path" or "raw_path"
            self.samples.append(line.split(", "))
        if split != "train":
            self.samples = self.samples
        self.transform = transform
        self.auto_padding_scale = auto_padding_scale
    ...
    def __getitem__(self, index):
        ...
        return rgb_jpeg, raw_img, raw_path
